-- Fix trek-bundling: alleen splitsen als len > 500m, niet bij >=400.
-- Hierdoor worden kleine restjes geabsorbeerd ipv aparte trek.
...
BEGIN
  ...
  -- Greedy chain: opeenvolgende stukken (gap < 2m) groeperen,
  -- splitsen alleen wanneer toevoeging > 500m maakt.
  chained AS (
    SELECT rn, g, len_m, sp, ep,
      SUM(CASE
        WHEN rn = 1 THEN 0
        ELSE 1
      END) OVER (ORDER BY rn) AS dummy
    FROM numbered
  ),
  -- Recursieve groep-toewijzing
  walk AS (
    SELECT rn, g, len_m, sp, ep,
           1 AS group_id,
           len_m AS acc_len,
    ...
    SELECT n.rn, n.g, n.len_m, n.sp, n.ep,
      CASE
        ...
        -- Toevoegen zou over 500m gaan EN huidige is al >= 200m → nieuwe groep
