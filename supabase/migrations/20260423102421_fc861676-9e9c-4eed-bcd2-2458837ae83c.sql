    ...
    SELECT
      ...
      COALESCE((
        ...
        WHERE s.trace_id = p_trace_id
          AND ST_DWithin(ST_Transform(s.geometry, 28992), pr.part_geom, 0.5)
      ), 999999) AS min_seq
    FROM parts_raw pr
  ),
  ordered AS (
...
CREATE OR REPLACE FUNCTION public.segments_with_part_idx(p_trace_id uuid)
 RETURNS TABLE(segment_id uuid, sequence integer, part_idx integer, bgt_feature_type text, bgt_subtype text, length_m numeric)
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
AS $function$
