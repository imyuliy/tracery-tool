...
BEGIN
  ...
  walk AS (
    SELECT rn, g, len_m, sp, ep, 1 AS group_id, len_m AS acc_len, ep AS group_end
    FROM numbered WHERE rn = 1
    UNION ALL
    SELECT n.rn, n.g, n.len_m, n.sp, n.ep,
      CASE
        WHEN ST_Distance(w.group_end, n.sp) > 2 THEN w.group_id + 1
        WHEN w.acc_len >= 400 AND (w.acc_len + n.len_m) > 500 THEN w.group_id + 1
        ELSE w.group_id
      END,
      CASE
        WHEN ST_Distance(w.group_end, n.sp) > 2 THEN n.len_m
        WHEN w.acc_len >= 400 AND (w.acc_len + n.len_m) > 500 THEN n.len_m
  ...
  grouped AS (
    SELECT group_id, ST_LineMerge(ST_Collect(g ORDER BY rn)) AS merged_geom,
           SUM(len_m) AS total_len, MIN(rn) AS first_rn
    FROM walk GROUP BY group_id
  ),
  ordered AS (
    SELECT (ROW_NUMBER() OVER (ORDER BY first_rn))::int - 1 AS pidx,
           merged_geom AS geom_28992, total_len::numeric AS len_m
    FROM grouped
  ),
  with_km AS (
