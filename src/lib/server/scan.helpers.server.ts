...
const MAX_TOKENS_PER_BATCH = 6000; // ruim voor 12-item JSON-array (~300 tok/item)
...
async function processBatch(input: BatchInput): Promise<SegmentNarrative[]> {
  ...
  const out: SegmentNarrative[] = segmentsWithEisen.map(({ seg, ctx, matches }, i) => {
    ...
    return {
      ...
      ai_aandacht_reden: toStrOrNull(item.aandacht_reden ?? item.reden),
      ai_voorgestelde_techniek: toStrOrNull(item.voorgestelde_techniek ?? item.techniek),
      aandacht_flags: ctx.autoFlags,
      aandacht_reden: ctx.autoReden,
      eisen_matches: matches,
